...
async function generateHardwareFingerprint() {
  ...
  try {
    ...
    if (ctx) {
      ...
      ctx.fillStyle = "#069";
      ctx.fillText("FLFingerprint", 2, 15);
      ctx.fillStyle = "rgba(102, 204, 0, 0.7)";
      ctx.fillText("FLFingerprint", 4, 17);
...
async function getHardwareFingerprint() {
  ...
  return new Promise(async (resolve) => {
    chrome.storage.local.get(["fl_hw_fingerprint"], async (res) => {
      if (res.fl_hw_fingerprint) {
        _cachedFingerprint = res.fl_hw_fingerprint;
        resolve(_cachedFingerprint);
      } else {
        try {
          const fp = await generateHardwareFingerprint();
          _cachedFingerprint = fp;
          chrome.storage.local.set({ fl_hw_fingerprint: fp });
          resolve(fp);
        } catch(e) {
          const fallback = crypto.randomUUID();
          _cachedFingerprint = fallback;
          chrome.storage.local.set({ fl_hw_fingerprint: fallback });
          resolve(fallback);
        }
      }
    });
